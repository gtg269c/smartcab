...
class LearningAgent(Agent):
	...
	def reset(self, destination=None, testing=False):
		...
		if testing:
			...
			#implements epsilon decay during training/learning
			self.epsilon = np.exp(-self.t*self.alpha*self.alpha*self.alpha*self.alpha)
	...
	def build_state(self):
		...
		state = tuple([inputs['oncoming'],inputs['light'],waypoint])
	...
	def choose_action(self, state):
		...
		if not self.learning:
			...
			if probab > self.epsilon:
				...
				print("MaxQ: ",maxQ)
				## get a list of all actions keys that match the maxQ
				action_list = [key for(key,val) in self.Q[state].items() if val == maxQ]
				
				## select the random action from a list of actions that have value of maxQ
